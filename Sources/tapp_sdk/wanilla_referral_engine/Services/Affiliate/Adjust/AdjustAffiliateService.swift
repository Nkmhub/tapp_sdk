...
public class AdjustAffiliateService: AffiliateService,AdjustSpecificService {
    ...
    public func initialize(environment: Environment, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        let adjustEnvironment = environment == .production ? ADJEnvironmentProduction : ADJEnvironmentSandbox
    ...
    public func handleEvent(eventId: String, authToken: String?) {
        ...
        if let event = ADJEvent(eventToken: eventId) {
            ...
            Logger.logError(ReferralEngineError.apiError(message: "Could not create ADJEvent with eventId \(eventId).", endpoint: ""))
        }
    }
    ...
    public func affiliateUrl(
        ...
        Logger.logInfo("Handling Adjust callback for custom URL... Not implemented yet.")
        completion(.failure(.unknownError(details: "Affiliate URL method not implemented for Adjust.")))
    }
    ...
    public func handleImpression(
        ...
        Logger.logInfo("Handle impression is not implemented. Use Tapp's method.")
        completion(.failure(ReferralEngineError.unknownError(details: "Impression handling not supported in AdjustAffiliateService.")))
    }
    ...
    public func getAttribution(completion: @escaping (ADJAttribution?) -> Void) {
        Adjust.attribution { attribution in
            if let attribution = attribution {
                ...
                Logger.logError(ReferralEngineError.unknownError(details: "No attribution available."))
            }
    ...
    public func trackThirdPartySharing(isEnabled: Bool) {
        guard let thirdPartySharing = ADJThirdPartySharing(isEnabled: NSNumber(value: isEnabled)) else {
            Logger.logError(ReferralEngineError.unknownError(details: "Failed to create ADJThirdPartySharing object."))
            return
        }
    ...
    public func trackAdRevenue(source: String, revenue: Double, currency: String) {
        if let adRevenue = ADJAdRevenue(source: source) {
            ...
            Logger.logError(ReferralEngineError.unknownError(details: "Failed to create ADJAdRevenue object for source: \(source)."))
        }
    }
    ...
    public func verifyAppStorePurchase(
        ...
        if let purchase = ADJAppStorePurchase(transactionId: transactionId, productId: productId) {
            ...
            Logger.logError(ReferralEngineError.unknownError(details: "Failed to create ADJAppStorePurchase object."))
    ...
    public func getAdid(completion: @escaping (String?) -> Void) {
        Adjust.adid { adid in
            if let adid = adid {
                ...
                Logger.logError(ReferralEngineError.unknownError(details: "No ADID available."))
            }
            completion(adid)
        }
    }
    ...
    public func getIdfa(completion: @escaping (String?) -> Void) {
        Adjust.idfa { idfa in
            if let idfa = idfa {
                ...
                Logger.logError(ReferralEngineError.unknownError(details: "No IDFA available."))
            }
            completion(idfa)
        }
    }
